...
app.get("/api/get/adminlogin", (req, res) => {
...
app.post("/api/post/supplier", (req, res) => {
    const {name, company, contact, email, stprice} = req.body;
    
    const sqlInsert = "INSERT INTO supplier (name, company, contact, email, stprice) VALUES (?, ?, ?, ?, ?)";
    db.query(sqlInsert, [name, company, contact, email, stprice], (error, result) => {
        if(error){
            console.log(error);
        }
    })
...
app.post("/api/post/order", (req, res) => {
    const {pdtname, category, quantity, picture, date, sprice} = req.body;
    
    const sqlInsert = "INSERT INTO shopapp.order (pdtname, category, quantity, pdtimage, date, sprice) VALUES (?, ?, ?, ?, ?, ?)";
    db.query(sqlInsert, [pdtname, category, quantity, picture.images, date, sprice], (error, result) => {
        if(error){
            console.log(error);
        }
    })
...
app.put("/api/orderupdate/:id", (req, res) => {
    const {id} = req.params;
    const {pdtname, category, quantity, picture, date, sprice} = req.body;
    const sqlUpdate = "UPDATE shopapp.order SET pdtname= ?, category= ?, quantity= ?, pdtimage= ?, date= ?, sprice = ? WHERE orderid = ?";
    db.query(sqlUpdate, [pdtname, category, quantity, picture.images, date, sprice, id], (error, result) => {
        if(error){
            console.log(error);
        }
        res.send(result);
